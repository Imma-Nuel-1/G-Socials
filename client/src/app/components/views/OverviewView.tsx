...
function MetricCard({ label, value, change, positive }: MetricCardProps) {
  return (
    <Card className="p-6">
      ...
      {change && (
        <div
          className={`flex items-center gap-1 text-xs ${positive ? "text-green-600" : "text-red-500"}`}
        >
          {positive ? (
...
function PlatformDistributionChart({ platformData }: { platformData: any[] }) {
  const hasData =
    platformData.length > 0 && platformData.some((p) => p.value > 0);
